...
export class PinterestExtractor extends BasePlatformExtractor {
  ...
  async extractMetadata(url: string): Promise<ContentMetadata> {
    try {
      ...
      const author = $('meta[property="og:site_name"]').attr('content') || 'Pinterest User';
      ...
      this.handleError(error, 'Failed to extract Pinterest metadata');
    }
  }
  ...
  async getDownloadUrl(
    ...
    try {
      ...
      this.handleError(error, 'Failed to get Pinterest download URL');
    }
  }
